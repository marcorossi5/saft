...
class Histo:
    ...
    def __init__(self, XMLHisto):
        self.Title, self.nbins, self.xmin, self.xmax = self.parse_description(
            XMLHisto.find("Description").text
        )
        (
            self.NEvents,
            self.TotalWeight,
            self.NEventsInHisto,
            self.TotalWeightInHisto,
        ) = self.parse_statistics(XMLHisto.find("Statistics").text)
        self.Underflow, self.Data, self.Overflow, self.DataNorm = self.parse_data(
            XMLHisto.find("Data").text
        )
        self.binning()
    ...
    def parse_description(self, DescriptionText):
        ...
        struct = [
            int((struct.split())[0]),
            float((struct.split())[1]),
            float((struct.split())[2]),
        ]
    ...
    def parse_data(self, DataText):
        ...
        return (
            ...
            sum(datatmp),
        )  # Need to consider over and underflow for proper normalization

    def binning(self):  # Determine the left hand limit of the bins
        self.binsize = (self.xmax - self.xmin) / self.nbins
        self.lbins = [
            self.xmin + self.binsize * i for i in range(self.nbins)
        ]  # range(self.xmin,self.xmax,self.binsize)#left limit of the bins

    ######## Printing

    def __repr__(self):
        return """Histogram: {}, {} bins in [{},{}]""".format(
            self.Title, self.nbins, self.xmin, self.xmax
        )
    ...
    def MathematicaList(
        self,
    ):  # provide an output that can make a Mathematica histogram
        points = zip(self.lbins, self.Data)
        spoints = ["{{{:f},{:f}}}".format(x, y) for (x, y) in points]
        spoints = ",".join(spoints)
    ...
    def __div__(self, H2):
        ...
        if self.nbins == H2.nbins and self.xmin == H2.xmin and self.xmax == H2.xmax:
            ...
            H3.NEventsInHisto /= H2.NEventsInHisto
            H3.TotalWeightInHisto /= H2.TotalWeightInHisto
            y0 = min([y for y in H2.Data if y != 0]) / 10.0
